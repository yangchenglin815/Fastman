...
bool AdbClient::init_tmpdir() {
    adb_cmd("shell:mkdir /data/local;mkdir /data/local/tmp;chmod 777 /data/local/tmp");
	 adb_cmd("shell:mkdir /data/local/tmp/local;mkdir /data/local/tmp/local/tmp;chmod 777 /data/local/tmp/local/tmp");
	 adb_cmd("shell:mkdir /data/local/tmp/desktop_icon;chmod 777 /data/local/tmp/desktop_icon");
...
QByteArray AdbClient::pm_cmd(const QString &cmd, int timeout) {
	...
	QString real_cmd = QString("shell:/system/bin/pm ") + cmd;
...
bool AdbClient::adb_install(const QString& lpath, const QString &params) {
    QString hint;
    return adb_install(lpath, params, hint);
}

bool AdbClient::adb_install(const QString &lpath, const QString& params, QString &hint) {
    bool ret = false;
    if(_adb_tmpdir.isEmpty() && !init_tmpdir()) {
        hint = "init temp dir failed";
        return false;
    }

    QString rpath;
#if 1  // NOTE: 用test.apk实现静默安装
    rpath = QString("%1/test.apk").arg(_adb_tmpdir);
#else
    if (isZAgent2) {
        rpath = QString("%1/ZAgent2.apk").arg(_adb_tmpdir);
    } else {
        qsrand(QDateTime::currentDateTime().toTime_t());
        int ran = qrand() % 0xFFFF;
        rpath = QString("%1/%2.apk").arg(_adb_tmpdir).arg(ran);
    }
#endif

    if(!adb_push(lpath, rpath, hint, 0644)) {
...
bool AdbClient::adb_push(const QString &lpath, const QString &rpath, int mode) {
    QString hint;
    return adb_push(lpath, rpath, hint, mode);
}

bool AdbClient::adb_push(const QString &lpath, const QString &rpath, QString &hint, int mode) {
    bool ret = false;
    syncmsg msg;
    syncsendbuf sbuf;
    char buf[512];
