...
void ZByteArray::append(char c) {
    _data = (char *)realloc(_data, _size+1);
    *(_data + _size) = c;
    _size += 1;
}

void ZByteArray::append(const char *str) {
    append((void *)str, strlen(str));
}

void ZByteArray::append(void *data, int len) {
    _data = (char *)realloc(_data, _size + len);
    memcpy(_data + _size, data, len);
    _size += len;
}

void ZByteArray::append(ZByteArray &other) {
    append(other.data(), other.length());
}

void ZByteArray::clear() {
    free(_data);
    _data = NULL;
    _size = 0;
}

void ZByteArray::remove(int from, int len) {
    if (from + len >= _size) {
        resize(from);
    } else {
        memmove(_data + from, _data + from + len, _size - (from + len));
        resize(_size - len);
    }
}

void ZByteArray::resize(int len) {
    if (len <= 0) {
        free(_data);
        _data = NULL;
        _size = 0;
...
void ZByteArray::chop(int len) {
    int new_size = _size - len;
    if(new_size < 0) {
        new_size = 0;
    }
    resize(new_size);
...
int ZByteArray::indexOf(const void *data, int len) {
    if(_data == NULL) {
        return -1;
    }
    char *p = (char *)memmem(_data, _size, (char *)data, len);
    return p == NULL ? -1 : p-_data;
}

int ZByteArray::indexOf(int from, const void *data, int len) {
    if(_data == NULL || from < 0 || from >= _size) {
        return -1;
    }
    char *p = (char *)memmem(_data+from, _size-from, (char *)data, len);
    return p == NULL ? -1 : p-_data;
}

bool ZByteArray::contains(const char *str) {
    if(_data == NULL || _size < 1) {
        return false;
    }

    return strstr(_data, str) != NULL;
}

bool ZByteArray::startsWith(const char *str) {
    if(_data == NULL || _size < 1) {
        return false;
    }

    return strstr(_data, str) == _data;
}

bool ZByteArray::endsWith(char c) {
    if(_data == NULL || _size < 1) {
        return false;
    }
...
char *ZByteArray::getNextUtf8(int &i) {
...
char *ZByteArray::getNextUtf8(int &i, int len) {
    char *ret = (char *) malloc(len + 1);
    int k = 0;
    while(len-- > 0) {
        ret[k++] = _data[i++];
...
//TODO works only for english chars
char *ZByteArray::getNextUtf16(int &i, int len) {
    char *ret = (char *) malloc(len + 1);
    u16 s;
    int k = 0;
    while(len-- > 0) {
...
void ZByteArray::putUtf8(char *src, bool addHead) {
    int len = strlen(src);
    if(addHead) {
        putValue(len);
    }
