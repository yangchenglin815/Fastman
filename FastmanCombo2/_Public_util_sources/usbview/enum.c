...
GUID hc_guid = {
   0x3ABF6F2DL,   0x71C4,   0x462A,   { 0x8a,   0x92,   0x1e,   0x68,
                                        0x61,   0xe6,   0xaf,   0x27 }
};

ZTreeItem
AddLeaf (
    ZTreeItem hTreeParent,
    LPARAM    lParam,
    ...
    switch (TreeIcon) {
        case ComputerIcon:
            sprintf(name, "[PC]%s", lpszText);
            break;
        case HubIcon:
            sprintf(name, "[HUB]%s", lpszText);
            break;
        case NoDeviceIcon:
            sprintf(name, "[NIL]%s", lpszText);
            break;
        case GoodDeviceIcon:
            sprintf(name, "[GOOD]%s", lpszText);
            break;
        case BadDeviceIcon:
        default:
            sprintf(name, "[BAD]%s", lpszText);
            break;
    }

    ret = ZTreeItemInsert(hTreeParent, name, (void *) lParam);
    return ret;
}
...
)
{
    ...
    //
    deviceInfo = SetupDiGetClassDevs((LPGUID)&hc_guid,
                                     NULL,
                                     NULL,
