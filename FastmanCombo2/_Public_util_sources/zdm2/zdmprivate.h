...
class ZDMPartThread : public QThread {
    ...
    ZDM *c_dm;
    ZDMTask *c_task;
    QList<ZDMPart *> c_parts;
public:
    ZDMPartThread(QObject *parent, ZDM *dm, ZDMTask * task, QList<ZDMPart *> part);
    ~ZDMPartThread();
    void run();
};
...
public:
    ...
    ZDownloadTask *addTaskFront(const QString& id, const QString &url, const QString &path);
    ZDownloadTask *addTask(const QString& url, const QString &path);
    ZDownloadTask *addTask(const QString& id, const QString& url, const QString &path,int use_thunder, QString *yunUrl = NULL);

    ZDownloadTask *insertTask(const QString &id, const QString &url, const QString &path, bool isUser);
